...
const views = {
  ["settings/index.html"]: async () => (await import('./index-ef28e8d7.js')).default,
  ["register/index.html"]: async () => (await import('./index2-fb63f725.js')).default,
  ["profile/index.html"]: async () => (await import('./index3-d64c3604.js')).default,
  ["login/index.html"]: async () => (await import('./index4-c5c699cf.js')).default,
  ["home/index.html"]: async () => (await import('./index5-236d603a.js')).default,
  ["home/components/popular-tags.html"]: async () => (await import('./popular-tags-aa677105.js')).default,
  ["editor/index.html"]: async () => (await import('./index6-3b8fac51.js')).default,
  ["components/tag-list.html"]: async () => (await import('./tag-list-bcdaf6c6.js')).default,
  ["components/pagination.html"]: async () => (await import('./pagination-78ac9b84.js')).default,
  ["components/markup.html"]: async () => (await import('./markup-b210955c.js')).default,
  ["components/layout.html"]: async () => (await import('./layout-f1eb70a1.js')).default,
  ["components/header.html"]: async () => (await import('./header-c2226e4a.js')).default,
  ["components/formatted-date.html"]: async () => (await import('./formatted-date-6a515484.js')).default,
  ["components/field.html"]: async () => (await import('./field-879c9503.js')).default,
  ["components/auth-errors.html"]: async () => (await import('./auth-errors-e6645788.js')).default,
  ["components/article-preview.html"]: async () => (await import('./article-preview-4e55746b.js')).default,
  ["article/index.html"]: async () => (await import('./index7-2e84d7f6.js')).default,
  ["article/components/comment-list.html"]: async () => (await import('./comment-list-8e4251d1.js')).default
};
const options = {
  service_worker: true,
...
async function render(view_name, props = {}) {
  ...
  const program = pipe(A.findFirst(possible_entries, (_) => _ in views), Effect.flatMap((entry) => {
    return Effect.tryPromise({
      try: () => views[entry](),
      catch: (e) => new ModuleError(`Unable to load module ${entry} for ${view_name}`, e)
    });
  }), Effect.flatMap((component) => {
    return pipe(Effect.try({
      try: () => component.render(props),
      catch: (e) => new ModuleError(`RenderError: ${view_name}`, e)
    }), Effect.map((rendered) => {
    ...
    var _a3;
    let html3 = error({ status: 404, message: (_a3 = e.message) !== null && _a3 !== void 0 ? _a3 : "View not found" });
    return Effect.succeed(html3);
  }), Effect.catchTag("ModuleLoadError", (e) => {
    console.log(e);
    let html3;
    {
      html3 = error({ status: 500, message: "Internal server error" });
    }
    return Effect.succeed(html3);
  }));
...
app.post("/login", async (ctx) => {
  ...
  let errors = pipe(
    ...
    Either.match({
      onLeft: (e) => {
        return "errors" in e ? e.errors : { unknown: ["An unknown error occurred"] };
      },
      onRight: constNull
    })
  );
  if (Either.isRight(data2)) {
    setCookie(ctx, "auth", JSON.stringify(data2.right.user));
    return ctx.redirect("/");
  }
...
app.post("/register", async (ctx) => {
  const form = await ctx.req.formData();
  const data2 = await pipe(
    register(Object.fromEntries(form)),
    Http.provide(Fetch.adapter, withApiUrl),
    Effect.either,
    Effect.runPromise
  );
...
app.get("/", async (ctx) => {
  ...
  const articles = pipe(
    tab === Tab.Personal ? getPersonalFeed({ offset: (page - 1) * 10 }) : tab === Tab.Global ? getArticles({ offset: (page - 1) * 10 }) : getArticles({ offset: (page - 1) * 10, tag })
  );
  const tags = pipe(
    getPopularTags(),
    Effect.map((_) => _.tags)
...
//# sourceMappingURL=index-8de5e0bc.js.map
